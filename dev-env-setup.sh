...
echo "=== Installing project development-only test tools ==="
"${NPM_BIN}" ci --ignore-scripts --no-audit --no-fund --prefix "${PROJECT_DIR}"
...
echo "  node --run format:check — check formatting"
echo "  node --run format  — rewrite formatting"
echo "  node --run test    — tests"
echo "  node --run test:coverage:server — server tests with coverage thresholds"
echo "  VIDIUM_CHROMIUM_PATH=/path/to/chromium node --run test:browser — browser tests"
